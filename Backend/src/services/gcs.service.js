...
const uploadFile = async ({
  ...
  try {
    ...
    return {
      ...
      fileExt: path.extname(originalName).toLowerCase().slice(1),
      category,
    };
  } catch (err) {
 ...
 */
const getSignedUrl = async (gcsObjectPath, ttlSeconds = GCS_SIGNED_URL_TTL_SECONDS) => {
  initializeGCS();
  ...
  try {
    ...
    console.error('Failed to generate signed read URL:', err);
